			...
			if (fields.size() > 0)
			{
				...
				for (const auto& [name, field] : fields)
				{
					...
					switch (field.Type)
					{
						...
						WRITE_SCRIPT_FIELD(Short,   uint16_t  );
						WRITE_SCRIPT_FIELD(Int,     uint32_t  );
						WRITE_SCRIPT_FIELD(Long,    uint64_t  );
					...
					if (scriptFields)
					{
						...
						for (auto scriptField : scriptFields)
						{
							...
							switch (type)
							{
								...
								READ_SCRIPT_FIELD(Short,   uint16_t  );
								READ_SCRIPT_FIELD(Int,     uint32_t  );
								READ_SCRIPT_FIELD(Long,    uint64_t  );
